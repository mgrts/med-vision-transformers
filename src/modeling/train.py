...
from src.config import (
    ...
    SEGMENTED_VAL_ANNOTATIONS_PATH,
    SEGMENTED_VAL_DATA_DIR,
    TEST_FRAC,
    TRACKING_URI,
    ULTRASOUND_DATA_DIR,
...
def get_model(base_model_name, num_classes, training_task, pre_trained_model_path=None):
    ...
    if training_task == "mim":
        model = MIMTransformer(base_model, dropout_rate=DROPOUT_RATE)
...
def get_loss_function(
    training_task, regression_loss_type=None, classification_loss=None, pos_weight=None
):
    if training_task == "mim":
        ...
        classification_loss = get_classification_loss_function(
            classification_loss, pos_weight=pos_weight
        )
...
def get_mmd_images(dataset, pools, batch_size=BATCH_SIZE):
    ...
    ood_loader = DataLoader(
        Subset(dataset, indices), batch_size=batch_size, shuffle=True, collate_fn=collate_fn
    )
    return next(iter(ood_loader))["pixel_values"].to(DEVICE)


def forward_batch(
    model, criterion, batch, training_task, apply_mmd=False, mmd_weight=MMD_WEIGHT, mmd_images=None
):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(train_loader, desc="Training", unit="batch"):
            ...
            loss, _ = forward_batch(
                model, criterion, batch, training_task, apply_mmd, mmd_images=mmd_images
            )
        ...
        if training_task in CLASSIFICATION_TASKS:
            clf_metrics, threshold = compute_binary_metrics(
...
def build_datasets(data_type):
    ...
    if data_type == "coco":
        train_aug = ImageDatasetCOCO(
            SEGMENTED_TRAIN_ANNOTATIONS_PATH, SEGMENTED_TRAIN_DATA_DIR, TRAIN_TRANSFORM
        )
        val_aug = ImageDatasetCOCO(
            SEGMENTED_VAL_ANNOTATIONS_PATH, SEGMENTED_VAL_DATA_DIR, TRAIN_TRANSFORM
        )
        train_eval = ImageDatasetCOCO(
            SEGMENTED_TRAIN_ANNOTATIONS_PATH, SEGMENTED_TRAIN_DATA_DIR, EVAL_TRANSFORM
        )
        val_eval = ImageDatasetCOCO(
            SEGMENTED_VAL_ANNOTATIONS_PATH, SEGMENTED_VAL_DATA_DIR, EVAL_TRANSFORM
        )
        official_test = ImageDatasetCOCO(
            SEGMENTED_TEST_ANNOTATIONS_PATH, SEGMENTED_TEST_DATA_DIR, EVAL_TRANSFORM
        )
    ...
    if data_type == "brats":
        pool_aug = ImageDatasetBrats(
            BRATS_TRAIN_DATA_DIR, BRATS_TRAIN_SURVIVAL_INFO_PATH, TRAIN_TRANSFORM
        )
        pool_eval = ImageDatasetBrats(
            BRATS_TRAIN_DATA_DIR, BRATS_TRAIN_SURVIVAL_INFO_PATH, EVAL_TRANSFORM
        )
        return pool_aug, pool_eval, _labels_to_int_array(pool_aug.labels), None, None
    ...
    if data_type == "brats_slice":
        ...
        return (
            pool_aug,
            pool_eval,
            _labels_to_int_array(pool_aug.labels),
            np.asarray(pool_aug.groups),
            None,
        )
...
def kfold_splits(indices, y, groups, n_splits, seed):
    ...
    if groups is not None:
        ...
        iterator = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=seed).split(
            indices, y_sub
        )
    else:
...
def main(
    ...
    with mlflow.start_run(experiment_id=experiment.experiment_id) as main_run:
        ...
        pos_weight = (
            ...
            if training_task in CLASSIFICATION_TASKS and n_pos > 0
            else None
        )
        ...
        mlflow.log_params(
            {
                ...
                "test_size": (
                    len(official_test) if official_test is not None else len(test_indices_in_pool)
                ),
        ...
        for split_num, (train_idx, val_idx) in enumerate(folds):
            ...
            with mlflow.start_run(run_name=f"Fold_{split_num + 1}", nested=True) as split_run:
                ...
                model = get_model(
                    base_model_name, num_classes, training_task, pre_trained_model_path
                )
                criterion = get_loss_function(
                    training_task, regression_loss_type, classification_loss_type, pos_weight
                )
        ...
        # Select the BEST fold (lowest validation loss), not one near the average. Use
        # nanargmin so a diverged fold (NaN val loss) is never picked; error if all diverged.
        fold_losses = np.asarray(overall_metrics["loss"], dtype=float)
        if np.all(np.isnan(fold_losses)):
            raise ValueError("All folds produced a NaN validation loss; cannot select a model.")
        best_fold = int(np.nanargmin(fold_losses))
        best_threshold = fold_thresholds[best_fold]
        logger.info(
            f"Selected fold {best_fold + 1} (lowest validation loss) as the exported model"
        )

        model = get_model(base_model_name, num_classes, training_task)
        model.load_state_dict(
            torch.load(artifacts_dir / f"fold_{best_fold}_model.pth", map_location=DEVICE)
        )
        model.to(DEVICE)
        ...
        if training_task in CLASSIFICATION_TASKS:
            test_subset = (
                official_test
                if official_test is not None
                else Subset(pool_eval, list(test_indices_in_pool))
            )
