...
from src.modeling.models import MultiLabelClassificationTransformer, MultiTaskTransformer
...
def main(
    ...
    # Use the decision threshold the training run tuned on its VALIDATION folds (logged as
    # `decision_threshold`), never one re-tuned on the test labels — re-tuning on test would
    # leak the test set into F1/precision/recall/accuracy. Fall back to 0.5 for older runs.
    thr_param = parent_run.data.params.get("decision_threshold")
    eval_threshold = float(thr_param) if thr_param is not None else 0.5
    logger.info(f"Scoring point metrics at the train-tuned decision threshold = {eval_threshold}")
    ...
    for run in child_runs:
        ...
        model_files = [
            a.path for a in client.list_artifacts(run.info.run_id) if a.path.endswith(".pth")
        ]
        if not model_files:
        ...
        if training_task == "classification":
            ...
            model = MultiTaskTransformer(
                base_model, image_size=IMAGE_SIZE, num_classes=num_classes
            )
        ...
        metrics, _ = compute_binary_metrics(y_true, y_prob, threshold=eval_threshold)
